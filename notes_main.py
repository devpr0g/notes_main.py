...
with open('notes_dump.json', 'w') as file:
...
button_create = QPushButton('Создать земетку')
button_del = QPushButton('Удалить земетку')
button_save = QPushButton('Сохранить земетку')
...
list_tags_label = QLabel('Список тэгов')
...
def save_note():
    if list_notes.selectedItems():
        key = key = list_notes.selectedItems()[0].text()
...
def del_tag():
    if list_tags.selectedItems():
        ...
        print('Тэг для удаления не выбран!')
...
button_tag_del.clicked.connect(add_tag)
button_tag_search.clicked.connect(search_tag)

notes_win.show()

with open('notes_dump.json', 'r') as file:
